...
static constexpr float AMPLIFIER_GAIN =
    CONFIG_AMMETER_AMPLIFIER_GAIN_X100 / 100.0f;
...
static constexpr float NOMINAL_VBAT_V =
    CONFIG_AMMETER_BATTERY_VOLTAGE_MV / 1000.0f;
...
void handler() {
  ...
  ESP_LOGI(TAG, "I=%.2fmA V=%.2fV P=%.2fmW Rem=%.1f%% adc_mv=%d (offset=%dmV)",
...
Measurement get_last_measurement() { return last_measurement; }
